...
export type EndCursor = Model.Snowflake | null;
...
export class Fetcher {
  ...
  async members(after: EndCursor): Promise<ResultPage<Model.GuildMember>> {
    ...
    const endpoint = `/guilds/${guildId}/members?after=${NullUtil.get(
      after
    )}&limit=${membersLimit}`;
  ...
  async messages(
    ...
    const endpoint = `/channels/${channel}/messages?after=${NullUtil.get(
      after
    )}&limit=${messagesLimit}`;
    const response = await this._fetch(endpoint);
    const results = response.map((x) => ({
  ...
  async reactions(
    ...
    const endpoint = `/channels/${channel}/messages/${message}/reactions/${emojiRef}?after=${NullUtil.get(
      after
